...
class App extends Component {
  ...
  calculateFaceLocation = (data) => {
    let clarifaiFace = data.outputs[0].data.regions.map(face => {
  ...
  onButtonSubmit = () => {
    this.setState({imageUrl: this.state.input});
      fetch('https://shrieking-goosebumps-86761.herokuapp.com/imageurl', {
        method: 'post',
      ...
      .then(response => {
        if (response) {
          fetch('https://shrieking-goosebumps-86761.herokuapp.com/image', {
            method: 'put',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
